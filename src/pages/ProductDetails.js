...
const ProductDetails = () => {
  ...
  const fetchProductDetails = async () => {
    try {
      ...
      // Fetch reviews
      const reviewResponse = await fetch(`${SummaryApi.getReviews.url.replace(":productId", params.id)}`);

      if (!reviewResponse.ok) {
        throw new Error(`Failed to fetch reviews: ${reviewResponse.status} ${reviewResponse.statusText}`);
      }
      const reviewData = await reviewResponse.json();
      if (reviewData.success) {
        setReviews(reviewData.reviews);
      } else {
        console.warn("No reviews found or error in fetching reviews");
      }
                ...
                reviews.map((review, index) => (
                  <div key={index} className="border p-4 mt-2 rounded shadow">
                    <div className="flex items-center">
                      ...
                      <p className="ml-2 text-gray-600">by {review.user?.name || "Anonymous"}</p>
                    </div>
